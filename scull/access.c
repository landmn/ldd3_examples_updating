...
static kuid_t scull_u_owner = { .val = 0 };	
...
static int scull_u_open(struct inode *inode, struct file *filp)
{
	...
	if (scull_u_count == 0)
		scull_u_owner.val = cred->uid.val; /* grab it */
...
static kuid_t scull_w_owner = { .val = 0 };	/* initialized to 0 by default */
...
static int scull_w_open(struct inode *inode, struct file *filp)
{
	...
	if (scull_w_count == 0)
		scull_w_owner.val = cred->uid.val; /* grab it */
	scull_w_count++;
	spin_unlock(&scull_w_lock);
